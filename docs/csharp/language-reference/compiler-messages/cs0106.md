...
-   La parola chiave [abstract](../../../csharp/language-reference/keywords/abstract.md) non è consentita in una dichiarazione esplicita dell'interfaccia, perché un'implementazione esplicita di un'interfaccia non può essere sottoposta a override.  
 ...
 Nelle versioni precedenti di Visual Studio non è consentito utilizzare il modificatore `static` in una classe, ma le classi `static` possono iniziare con [!INCLUDE[vsprvsext](../../../csharp/language-reference/compiler-messages/includes/vsprvsext-md.md)].  
