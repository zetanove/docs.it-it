...
> [!NOTE]
> .NET Framework 4.7 è preinstallato in tutti i sistemi Windows 10 Creators Update. Non è disponibile per il download o l'installazione in altri sistemi operativi Windows.  
...
Per un elenco delle nuove API aggiunte a [!INCLUDE[net_v462](../../../includes/net-v462-md.md)], vedere l'argomento sulle [modifiche apportate all'API di .NET Framework 4.6.2](https://github.com/Microsoft/dotnet/blob/master/releases/net462/dotnet462-api-changes.md) su GitHub. Per un elenco dei miglioramenti apportati alle funzionalità e delle correzioni di bug in [!INCLUDE[net_v462](../../../includes/net-v462-md.md)], vedere l'argomento sulle [modifiche apportate all'API di .NET Framework 4.6.2](http://go.microsoft.com/fwlink/?LinkId=708778) su GitHub. Per altre informazioni, vedere l'[annuncio di .NET Framework 4.6.2](https://blogs.msdn.microsoft.com/dotnet/2016/08/02/announcing-net-framework-4-6-2/) in .NET Blog.  
 ...
 **Supporto SignedXml per la generazione di hash SHA-2**  
 In [!INCLUDE[net_v462](../../../includes/net-v462-md.md)] è stato aggiunto il supporto per la classe <xref:System.Security.Cryptography.Xml.SignedXml> per i metodi di firma PKCS#1 per RSA-SHA256, RSA-SHA384 e RSA-SHA512 e per gli algoritmi con classificazione di riferimento SHA256, SHA384 e SHA512.  
...
### <a name="adonet"></a>ADO.NET  
 ...
 Miglioramento del comportamento di connessione di <xref:System.Data.SqlClient.SqlConnectionStringBuilder.MultiSubnetFailover%2A> per Always On  
...
### <a name="windows-presentation-foundation-wpf"></a>Windows Presentation Foundation (WPF)  
 ...
 Il ritardo di attivazione degli eventi tocco è stato risolto in [!INCLUDE[net_v461](../../../includes/net-v461-md.md)]. Inoltre, l'immissione di un controllo <xref:System.Windows.Controls.RichTextBox> non blocca più il thread di rendering durante l'input veloce.  
...
-   Ridimensionamento nei controlli Windows Form. È possibile usare l'impostazione DPI per ridimensionare i componenti dei controlli (ad esempio, le icone visualizzate in una griglia delle proprietà) scegliendo una voce nel file di configurazione dell'applicazione (app.config) relativo alla propria app. Questa funzionalità è attualmente supportata nei seguenti controlli Windows Form:  
     ...
     Alcuni aspetti di <xref:System.Windows.Forms.DataGridView>. Per informazioni sugli altri controlli supportati, vedere l'argomento sulle [nuove funzionalità della versione 4.5.2](#v452)  
