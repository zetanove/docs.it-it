...
I tipi disponibili in .NET Framework e nelle librerie di classi di altri produttori possono fornire metodi che consentono di continuare a eseguire un'applicazione mentre le operazioni asincrone vengono eseguite in thread diversi da quello principale dell'applicazione.  Nelle sezioni indicate di seguito vengono illustrati esempi di codice relativi ai diversi sistemi con cui è possibile chiamare i metodi asincroni che utilizzano il modello di progettazione <xref:System.IAsyncResult>.  
